...
function ∂objective_function(r_array, θ_array)
    ...
    return Zygote.gradient(
        ...
        collect(r_array),
        collect(θ_array),# TODO may be unnecessary
    )
end

# TODO: pick a scattering cross section as target
function main()
    n_max = 1
    #
    wl_or_freq_input = 1e-6
    input_unit = "m"
    Eps_r_r_1, Eps_r_i_1 = 1.0, 0.0
    Eps_r_r_2, Eps_r_i_2 = 1.5, 0.01
    Mu_r_r_1, Mu_r_i_1 = 1.0, 0.0
    Mu_r_r_2, Mu_r_i_2 = 1.0, 0.0
    ...
    for n_iteration in 1:50
        ...
        ∂loss = ∂objective_function(r_array, θ_array) #TODO square sum of losses
...
################################################################################
function objective_complex(r_array_input, θ_array_input)
    # println(r_array)
    # println("I am <<objective_function>>, and the type of r_array is $(typeof(r_array))")
    # println("r_array = $r_array")
